           ...
           if(fb.length > 0) {
                fb.each(function(){
                    ...
                    if(isset(conf[name]) && isset(conf[name].cfg.antispamjs)) {
                      $(form).prepend('<input type="text" name="'+ conf[name].cfg.antispamjs +'" value="tesby" style="display:none;position: absolute;">');
                    }
                });
            }
  })();
        ...
        success: function(answer) {
          ...
          if(isset(cfg.notify) && !/none/i.test(cfg.notify)) {
             ...
             if(/textbox/i.test(cfg.notify)) {
                 ...
                 $(vars.form).find('input[type=text]:visible, textarea:visible, select:visible').removeClass('input-error');
                 if(isset(answer.errors)) {
                     $.each(answer.errors, function(k,val) {
                         var reg = /[a-z]/i;
                         if(reg.test(k)) {
